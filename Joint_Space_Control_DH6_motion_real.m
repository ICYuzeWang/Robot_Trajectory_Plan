...
quarbox = zeros(8,3); %8 by 3 mesh
quarbox(1,:)=[0.09/2, 0.085/2,0];
quarbox(2,:)=[-0.09/2, 0.085/2,0];
quarbox(3,:)=[0.09/2, -0.085/2,0];
quarbox(4,:)=[-0.09/2, -0.085/2,0];
quarbox(5,:)=[0.09/2, 0.085/2,0.1025];
quarbox(6,:)=[-0.09/2, 0.085/2,0.1025];
quarbox(7,:)=[0.09/2, -0.085/2,0.1025];
quarbox(8,:)=[-0.09/2, -0.085/2,0.1025];

collrota=collisionMesh(quarbox);
collrota.Pose=trvec2tform([0,0,-0.1025]);
coll1 = collisionBox(0.105,0.025,0.053);  % box:length,width,height (x,y,z)
coll1.Pose = trvec2tform([0.105/2 0 0]);  % centre
coll2 = collisionBox(0.097,0.025,0.053);  % box:length,width,height (x,y,z)
coll2.Pose = trvec2tform([0.097/2 0 0]);  % centre
coll3 = collisionBox(0.05,0.06,0.05);  % box:length,width,height (x,y,z)
coll3.Pose = trvec2tform([0.05/2 0.03 0]);  % centre
...
step4_position = [3.14159 1.5708 0 3.14159 3.14159 1.57080]./pi.*180;
% step4_position = [0 2.13643-1.57080 0.51605 1.27645 1.57080 1.57080]./pi.*180;
position_difference = step4_position - current_position;
%% Record Video

homeconfig = homeConfiguration(robot)
Initial = homeconfig;
Final = [1.5708 1.5708 -1.5708 0 1.5708 0]';
% Final = [pi/2 0 0 0 0]';
...
% real
step_diff = position_difference / Step;
...
writerObj = VideoWriter('.\figure\task2_DH6_real.avi');
...
saveas(a,'.\figure\Task2_DH6_real_angle.png')
%%
a = figure;
...
saveas(a,'.\figure\Task2_DH6_real_angle_velocity.png')
%%
a = figure;
plot3(Point_position_matrix(1, :), Point_position_matrix(2, :), Point_position_matrix(3, :));
saveas(a,'.\figure\Task2_DH6_real_trajectory.png')
...
saveas(a,'.\figure\Task2_DH6_real_linear_velocity.png')
% % Plot of joint velocities during motion
% subplot(3,1,1)
